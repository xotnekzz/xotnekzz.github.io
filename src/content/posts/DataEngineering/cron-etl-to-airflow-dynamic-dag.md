...
**1. 유연한 날짜 처리 (initialize_date)**  
스케쥴러에 의해 자동으로 돌 때는 execution_date를 사용하지만, 장애 발생으로 재입력이 필요한 경우 수동으로 특정 날짜를 돌려야 할 때 Airflow Params(date)를 우선순위로 받고 입력하도록 처리하였습니다. 이를 통해 Backfill이 매우 간편해졌습니다.  
  
**2. 데이터 무결성 검증 (validate)  
**데이터를 무작정 적재하지 않습니다. 태스크내에 검증로직을 추가하여 오염된 지표가 입력되지 않도록 합니다.  
이는 BI 대시보드의 신뢰도를 높이는 일이라고 생각합니다.  
  
**3. 멱등성 및 대용량 처리 ( delete_and_load )**
...
| [Apache Kafka [1] - 카프카의 배경과 근본](/posts/DataEngineering/apache-kafka-background-and-fundamentals) (0) | 2025.12.17 |
| --- | --- |
| [StarRocks의 압도적 퍼포먼스 경험기](/posts/DataEngineering/starrocks-performance-experience) (0) | 2025.12.05 |
| [Airflow 살펴보기](/posts/DataEngineering/airflow-overview) (0) | 2025.12.01 |
